...
- 基于本项目代码撰写文章或提供功能截图时，先使用对应代码版本、README 中的配置方式、依赖和正常入口完成真实环境验证，再描述功能结果。
- 从实际初始化、数据库迁移和启动命令开始，验证文章涉及的完整用户操作路径。独立测试数据库和无敏感演示数据通过项目正常配置接入，并记录环境版本、命令、输入和输出。
- 项目启动或关键流程失败时，保留真实错误并报告阻塞，相关功能结论保持待验证。不能通过另建临时 Harness、替换入口、注入模型响应或服务、浏览器请求重定向等手段绕过问题，再把截图当作正常项目运行的证据。
- 自动化测试继续使用 mock 和隔离数据；其结果明确归为测试证据。文章中的真实功能验证使用项目实际配置的模型、数据库及其他服务，按已获授权的范围执行；环境或授权缺失时报告缺口，等待真实验证条件具备。
- 功能截图直接来自真实操作过程；预览界面、fixture、手工注入页面和由日志生成的整理视图应明确标注来源与范围。它们不能替代正常启动、实际模型调用或端到端验证。
- 分别报告测试、构建、数据库操作和页面流程的结果。后端保存成功而页面失败时，应说明两者的实际状态；关键流程未通过的文章保持草稿，修复后从正常入口重新验证。
- 证据保留对应提交、环境与依赖版本、操作步骤和原始记录。截图可遮盖敏感信息，但不得改写结果或隐藏影响结论的错误。文章和截图准备工作本身不授权提交、推送或发布。
...
- 自动化测试不得依赖真实 OpenAI/Tavily 请求。使用 mock 构造 typed events、完整 Response、同轮多个 function calls 和错误事件；文章中的真实环境功能验证另行执行，分别报告证据范围。
