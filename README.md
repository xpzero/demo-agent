...
- 上下文使用 `len(text)` 估算预算；滚动摘要可能逐次蒸馏损失早期细节。摘要失败期间只对游标后的原话做截断，未收编的中间段暂时不可见；原始消息仍在 SQLite。单条原文超过单次收编预算时会分段摘要，全部成功后才推进游标；分段摘要需要多次模型请求。
...
| `SUMMARY_ABSORB_BUDGET` | 8000 字符 | 单次收编上限；超长单条消息分段处理 |
